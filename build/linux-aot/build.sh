...
docker cp temp:/app/aws-backup/publish ./linux-aot-output
docker cp temp:/app/aws-backup-commands/publish ./linux-aot-output
docker cp temp:/app/cert-gen/bin/publish ./linux-aot-output
docker rm temp
docker system prune -af